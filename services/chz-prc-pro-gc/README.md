...
Process Layer API diseñada para el ecosistema MiPro, optimizada para ejecutarse en **Google Cloud Run**.
...
```bash
gcloud run deploy chz-prc-mipro-api-dev \
  --source . \
  --region us-east1 \
...
```bash
gcloud run deploy chz-prc-mipro-api \
  --source . \
  --region us-east1 \
  --project chz-cynx-space-ia \
